...
public abstract class PhysicsComponent implements Component {
    ...
    protected void initBoundingBox(float percentageWidthReduced, float percentageHeightReduced){
        ...
        float minX = 0;
        float minY = 0;
        setMinXY(minX,minY,_nextEntityPosition);
        ...
        switch(_boundingBoxLocation){
            case BOTTOM_LEFT:
                ...
                break;
            case BOTTOM_CENTER:
                _boundingBox.setCenter(minX + origWidth/2,
                        minY + origHeight/4);
                break;
            case CENTER:
                _boundingBox.setCenter(minX + origWidth/2,
    ...
    private void setMinXY(float minX, float minY, Vector2 vector2){
        if( Map.UNIT_SCALE > 0 ) {
            minX = vector2.x / Map.UNIT_SCALE;
            minY = vector2.y / Map.UNIT_SCALE;
        }else{
            minX = vector2.x;
            minY = vector2.y;
        }
