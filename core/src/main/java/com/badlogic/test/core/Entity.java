...
//    private static final String _defaultSpritePath = "assets/sprites/characters/Warrior.png";
    private static final String _defaultSpritePath = "assets/newSprites/player_sprite_sheet_alpha.png";
    ...
    private TextureRegion _standRightFrame;
    private TextureRegion _standUpFrame;
    private TextureRegion _standLeftFrame;
    private TextureRegion _standDownFrame;
    ...
    public final int ANIMATION_FRAMES = 4;
    ...
    private void loadAllAnimations(){
        ...
        Texture texture = Utility.getTextureAsset(_defaultSpritePath);
        TextureRegion[][] textureFrames = TextureRegion.split(texture, FRAME_WIDTH, FRAME_HEIGHT);
        _walkDownFrames = new Array<TextureRegion>(ANIMATION_FRAMES);
        _walkLeftFrames = new Array<TextureRegion>(ANIMATION_FRAMES);
        _walkRightFrames = new Array<TextureRegion>(ANIMATION_FRAMES);
        _walkUpFrames = new Array<TextureRegion>(ANIMATION_FRAMES);
        _standDownFrame = null;
        _standLeftFrame = null;
        _standRightFrame = null;
        _standUpFrame = null;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                switch(i)
                {
                    case 0:
                        if (j == 0) {
                            _standRightFrame = region;
                        } else {
                            _walkRightFrames.insert(j-1, region);
                        }
                        break;
                    case 1:
                        if (j == 0) {
                            _standUpFrame = region;
                        } else {
                            _walkUpFrames.insert(j-1, region);
                        }
                        break;
                    case 2:
                        if (j == 0) {
                            _standDownFrame = region;
                        } else {
                            _walkDownFrames.insert(j-1, region);
                        }
                        break;
                    case 3:
                        if (j == 0) {
                            ...
                            _walkLeftFrames.insert(j-1, region);
                        }
                        break;
                }
            }
        }
        _walkDownAnimation = new Animation<>(ANIMATION_FRAME_DURATION, _walkDownFrames, Animation.PlayMode.LOOP);
        _walkLeftAnimation = new Animation<>(ANIMATION_FRAME_DURATION, _walkLeftFrames, Animation.PlayMode.LOOP);
        _walkRightAnimation = new Animation<>(ANIMATION_FRAME_DURATION, _walkRightFrames, Animation.PlayMode.LOOP);
        _walkUpAnimation = new Animation<>(ANIMATION_FRAME_DURATION, _walkUpFrames, Animation.PlayMode.LOOP);
    }
